...
import { Ct as require_jsx_runtime, Dt as useToast, Ft as require_react, Mt as useNavigate, Ot as Link, Q as LoaderCircle, Rt as __toESM, Z as Mail, k as supabase, st as createLucideIcon, v as Button, w as useAuth } from "./index-CV_nKZ9-.js";
...
function AdminLogin() {
	...
	const handleLogin = async (e) => {
		...
		try {
			...
			if (!isAuthorized) try {
				const { data: isAdminDB, error: rpcError } = await supabase.rpc("is_admin");
				if (!rpcError && isAdminDB) isAuthorized = true;
				else if (rpcError) console.warn("RPC Admin check failed:", rpcError);
			} catch (err) {
				console.warn("Unexpected error during admin RPC check:", err);
			}
			if (!isAuthorized) {
				await signOut();
				console.warn(`Access denied for user ${data.user.email} with role: ${currentRole}`);
...
//# sourceMappingURL=AdminLogin-CyRBBIel.js.map